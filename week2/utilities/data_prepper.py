...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        # Your structure should look like the data frame below
            ...
            hits = response['hits']['hits']
            for doc in hits:
                feature_results["doc_id"].append(int(doc['_id']))  # capture the doc id so we can join later
                feature_results["query_id"].append(query_id)
                feature_results["sku"].append(int(doc['_source']['sku'][0]))  # ^^^
                for feature in doc['fields']['_ltrlog'][0]['log_entry']:
